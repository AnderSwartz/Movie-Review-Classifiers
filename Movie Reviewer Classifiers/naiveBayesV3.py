...
def rel_freq(word,label):
    if(label==0):
        return (negVocab.get(word,0)+lSmoothing)/((lSmoothing*index)+negVocabTokenSize+posVocabTokenSize)
    else:
        return (posVocab.get(word,0)+lSmoothing)/((lSmoothing*index)+posVocabTokenSize+negVocabTokenSize)
